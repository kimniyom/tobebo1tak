...
class storeimg extends CI_Controller {
    ...
    public function __construct() {
        parent::__construct();
        $this->load->model('tobestoreimages_model', 'model');
   ...
   public function Auth() {
    ...
    public function index($type) {
        ...
        $sql = "select * from tobestoreimages_folder where user_id = '$user_id' AND type = '$type' limit 1";
    ...
    public function createfolder() {
        ...
        $ref = md5($userid . "tobe_" . date("YmdHis") . $foldername);
        ...
        if (!is_dir($pathFolder)) {//เช็คว่ามีชื่อโฟล์เดอร์นี้หรือไม่
            ...
            $this->db->insert("tobestoreimages_folder", $columns);
            echo "1";
        } else {
            echo "0";
        }
    ...
    public function updatefolder() {
        ...
        $this->db->where("id", $id);
        $this->db->update("tobestoreimages_folder", $columns);
    }
    ...
    public function viewfolder() {
        ...
        $result = $this->db->get_where('tobestoreimages_list', array("folder_id" => $id))->row();
        $data['sum'] = "<em>size " . number_format($result->size_mb, 2) . "MB</em>";
        $data['folder'] = $this->db->get_where('tobestoreimages_folder', array('id' => $id))->row();
        $sql = "select * from tobestoreimages_list WHERE folder_id = '$id'";
        ...
        $this->load->view('toberegis/storeimg/listimages', $data);
    }

    public function uploadfile($folder_id) {
        $row = $this->db->get_where("tobestoreimages_folder", array("id" => $folder_id))->row();
        ...
        if (!empty($_FILES)) {
            ...
            if (in_array($fileParts['extension'], $fileTypes)) {
                ...
                $this->db->insert('tobestoreimages_list', $data);
    ...
    public function Getdata() {
        ...
        if ($id != "") {
            $data['folder_id'] = $id;
            $data['row'] = $this->db->get_where("tobestoreimages_folder", array("id" => $id))->row();
            $data['images'] = $this->db->get_where("tobestoreimages_list", array("folder_id" => $id));
        }
        if ($data['row']->type == "0") {
            $page = "toberegis/storeimg/images";
        } else {
            $page = "toberegis/storeimg/file";
        }
        $this->load->view($page, $data);
    }
    ...
    public function editimagename() {
        ...
        $this->db->where("id", $id);
        $this->db->update("tobestoreimages_list", $columns);
    }

    public function deleteimages() {
        $id = $this->input->post('id');
        $rs = $this->db->get_where('tobestoreimages_list', array('id' => $id))->row();
        $folder = $this->db->get_where('tobestoreimages_folder', array('id' => $rs->folder_id))->row();

        unlink('upload_images/' . $folder->ref . "/thumb/" . $rs->images);
        unlink('upload_images/' . $folder->ref . "/" . $rs->images);

        $this->db->where("id", $id);
        $this->db->delete("tobestoreimages_list");
    }

    public function deletefolder() {
        $id = $this->input->post('id');
        $folder = $this->db->get_where('tobestoreimages_folder', array('id' => $id))->row();
        $result = $this->db->get_where('tobestoreimages_list', array('folder_id' => $id));
        ...
        $this->db->where("id", $id);
        $this->db->delete("tobestoreimages_folder");
    }
