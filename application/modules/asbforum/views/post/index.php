...
<div class="row">
    ...
    <div class="col-md-9 col-lg-9">
        <textarea class="form-control text-area" id="detail" rows="5"></textarea>
    </div>
</div>
...
<div class="row">
    ...
    <div class="col-md-3 col-lg-3" style=" padding-top: 10px;">
        <input type="radio" name="privilege" id="privilege" value="0" checked="checked"/> ทุกคนมองเห็น
        <input type="radio" name="privilege" id="privilege" value="1"/> เฉพาะสมาชิก
    </div>
</div>
...
<script type="text/javascript">
    function ChageSubcategory(catid) {
        var url = "<?php echo site_url('asbforum/post/getsubcategory') ?>";
    ...
    function Post() {
        var url = "<?php echo site_url('asbforum/post/posts') ?>";
        ...
        var data = {title: title, category: category, subcategory: subcategory,detail: detail, privilege: privilege};

        $.post(url, data, function (datas) {
            var post_id = data.post_id;
            var urlredir = "<?php echo site_url('asbforum/post/view') ?>" + "/" + post_id;
