...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        /*
         ...
         'uploader': '<?php //echo site_url('backend/pages/updateuploadfile/' . $id)     ?>', //เมื่อ submit แล้วให้ action ไปที่ไฟล์ไหน
...
<div class="well well-sm">
    <label>หัวข้อ</label>
    <input type="text" id="title" name="title" required="required" class="form-control input-sm" value="<?php echo $datas->title ?>"/>
    <input type="hidden" id="group" name="group" value="<?php echo $head; ?>"/>
    <label>รายละเอียด</label>
    <textarea id="detail" rows="5" class="form-control"><?php echo $datas->detail ?></textarea>
    ...
    <!--
    Update by kimniyom 2018-02-09
    <label>แนบไฟล์ *.zip; *.rar; *.ppt; *.pdf; *.doc</label>
    <input id="file_upload" name="file_upload" type="file" multiple>
    <input type="hidden" id="f" />
    <?php //if ($datas->file) {  ?>
        <a href="<?php //echo base_url()    ?>file_download/<?php //echo $datas->file    ?>" target="_blank">
            <i class="fa fa-file-zip-o"></i> ไฟล์แนบ <?php //echo $datas->file     ?></a>
        <hr/>
    <?php //}  ?>
    -->
    <br/><br/><label>ผู้เผยแพร่</label>
    ...
    <br/>
    <button type="button" class="btn btn-success" onclick="Save()"><i class="fa fa-save"></i> แก้ไขข้อมูล</button>
</div>
...
<script type="text/javascript">
    ...
    function Save() {
        ...
        var url = "<?php echo site_url('backend/pages/saveupdate') ?>";
        var title = $("#title").val();
        var group = $("#group").val();
        var id = "<?php echo $id ?>";
