...
$list = array(
    ...
    array('url' => 'backend/news/view/' . $news->id, 'label' => $news->titel),
...
<form id="from" name="from">
    ...
    <label>หัวข้อ</label>
    <input type="text" id="_title" name="_title" style="width:98%;" required="required" value="<?php echo $news->titel ?>" class="form-control input-mini"/>
...
<script type="text/javascript">
    ...
    function edit_news() {
        ...
        $.ajax({
            ...
            data: data,
            success: function () {
                window.location = "<?php echo site_url('backend/news/view/' . $news->id) ?>";
            }
        });
    }
