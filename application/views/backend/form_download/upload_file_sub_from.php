<script type="text/javascript">
    $(document).ready(function () {
        var mas_id = "<?php echo $mas_id ?>";
        $('#file_upload').uploadify({
            'buttonText': 'กรุณาเลือกไฟล์ ...',
            'auto': true, //เปิดใช้การอัพโหลดแบบอัติโนมัติ
            'swf': '<?= base_url() ?>lib/images/uploadify.swf', //โฟเดอร์ที่เก็บไฟล์ปุ่มอัพโหลด
            'uploader': '<?= site_url('backend/form_download/file_upload_sub_from/' . $sub_id) ?>', //เมื่อ submit แล้วให้ action ไปที่ไฟล์ไหน
            ...
            'onUploadComplete': function (success) { //เมื่ออัพโหลดเสร็จแล้วให้เรียกใช้งาน function load()
                window.location = "<?php echo site_url('backend/form_download/get_sub_from/') ?>" + "/" + mas_id;
            }
        });
    });
</script>
...
$list = array(
    ...
    array('url' => 'backend/form_download/get_sub_from/' . $mas_id, 'label' => $masform)
);
