...
<script type="text/javascript">
    $(document).ready(function () {
        $('#file_upload').uploadify({
            ...
            'uploader': '<?= site_url('backend/storeimg/' . $upload . '/' . $folder->id) ?>', //เมื่อ submit แล้วให้ action ไปที่ไฟล์ไหน
...
<script type="text/javascript">
    ...
    function saveedit() {
        var url = "<?php echo site_url('backend/storeimg/updatefolder') ?>";
    ...
    function getdata(folder_id) {
        var url = "<?php echo site_url('backend/storeimg/getdata') ?>";
    ...
    function deletefolder(id) {
        ...
        if (r == true) {
            var url = "<?php echo site_url('backend/storeimg/deletefolder') ?>";
            var data = {id: id};
            $.post(url, data, function (datas) {
